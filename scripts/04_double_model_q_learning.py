...
board = SummaryWriter('../log/double_model_q')
gamma = 0.8
...
for game_count in itertools.count():
    ...
    while True:
        ...
        expect = torch.tensor([reward]).float() if terminate or truncated else reward + gamma * torch.max(target_model(torch.FloatTensor(np.array([next_state]))), dim=1)[0].detach()
